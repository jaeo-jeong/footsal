...
일반 실행에서는 자동 선택을 유지한다. 코어 준비가 실패했을 때에만 이전 Worker를 종료하고 `tesseract-core-lstm.wasm.js`를 지정한 새 Worker로 한 번 재시도한다. 특정 코어를 지정할 수 있는 경로는 [7.0.0 코어 로더](https://github.com/naptha/tesseract.js/blob/v7.0.0/src/worker-script/browser/getCore.js)에 따른다. Worker 자체를 불러오지 못한 경우에는 같은 실행을 반복하지 않고 파일 상태를 확인한다. 확인 요청도 같은 사이트로만 보내며 사진을 포함하지 않는다.
